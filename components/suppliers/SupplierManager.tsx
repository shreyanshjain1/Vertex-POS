...
type Supplier = { id: string; name: string; contactName: string | null; email: string | null; phone: string | null; address: string | null; notes: string | null; isActive: boolean; _count?: { purchases: number } };
...
export default function SupplierManager({ initialSuppliers }: { initialSuppliers: Supplier[] }) {
  ...
  async function toggleSupplier(supplier: Supplier) {
    const response = await fetch(`/api/suppliers/${supplier.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ isActive: !supplier.isActive, ...supplier }) });
    ...
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      ...
      <Card>
        ...
        <div className="mt-4 space-y-3">
          {suppliers.length ? suppliers.map((supplier) => <div key={supplier.id} className="rounded-2xl border border-stone-200 p-4"><div className="flex items-start justify-between gap-4"><div><div className="font-semibold text-stone-900">{supplier.name}</div><div className="text-sm text-stone-500">{supplier.contactName || 'No contact'} • {supplier.phone || 'No phone'}</div><div className="text-xs text-stone-500">{supplier.email || 'No email'} • {supplier._count?.purchases ?? 0} purchase(s)</div></div><Button type="button" variant="secondary" onClick={() => toggleSupplier(supplier)}>{supplier.isActive ? 'Archive' : 'Activate'}</Button></div></div>) : <div className="rounded-2xl border border-dashed border-stone-300 bg-stone-50 p-5 text-sm text-stone-500">No suppliers yet.</div>}
        </div>
      </Card>
    </div>
  );
