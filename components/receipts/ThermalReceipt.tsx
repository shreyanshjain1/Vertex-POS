...
export default function ThermalReceipt({
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="receipt-shell mx-auto w-full max-w-[380px] rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
        <div className="border-b border-dashed border-stone-300 pb-4 text-center">
          <div className="text-lg font-black text-stone-900">{receiptHeader || shop.name}</div>
          {shop.address ? <div className="mt-1 text-xs text-stone-600">{shop.address}</div> : null}
          <div className="mt-1 text-xs text-stone-600">
            {shop.phone || '—'} {shop.email ? `• ${shop.email}` : ''}
          </div>
        </div>

        <div className="space-y-1 border-b border-dashed border-stone-300 py-4 text-xs text-stone-700">
        ...
        {sale.notes ? (
          <div className="border-b border-dashed border-stone-300 py-4 text-xs text-stone-600">
